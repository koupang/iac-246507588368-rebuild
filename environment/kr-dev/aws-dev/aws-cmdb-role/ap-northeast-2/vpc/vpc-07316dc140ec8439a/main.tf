...
module "nat_gateway" {
  ...
  allocation_id     = each.value.connectivity_type == "public" ? module.legacy_eip.new_eips[each.key] : null
  subnet_id         = module.legacy_nat_gateway.new_nat_subnets[each.key]
  tags              = each.value.tags
}
