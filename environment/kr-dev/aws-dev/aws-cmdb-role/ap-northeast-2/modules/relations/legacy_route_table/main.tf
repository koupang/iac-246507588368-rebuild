locals {
  ...
  new_route_tables = {
    for route_table, route_table_id in var.route_table_id_with_legacy_relations : route_table => lookup(local.rt_mapping, route_table_id, route_table_id)
  }
}

output "route_table_id_with_legacy_relations" {
  value = var.route_table_id_with_legacy_relations
}
