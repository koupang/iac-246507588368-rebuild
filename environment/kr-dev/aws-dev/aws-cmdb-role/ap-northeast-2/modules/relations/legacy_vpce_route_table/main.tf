locals {
  ...
  new_route_tables = {
    for route_table, route_table_ids in var.vpce_rt_with_legacy_relations :
    route_table => [ for route_table_id in route_table_ids : lookup(local.rt_mapping, route_table_id, route_table_id)]
  }
}

output "vpce_rt_with_legacy_relations" {
  value = var.vpce_rt_with_legacy_relations
}
